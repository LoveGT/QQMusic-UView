<template>
	<view class="video-item w-345 mb-2 rounded-1 mb-4" @click="handleClickItem">
		<view class="album relative overflow-hidden rounded-1">
			<van-image width="100%" fit="widthFix" :src="item.cover" />
			<view
				class="info w-100% text-white text-24 absolute bottom-15 flex justify-between"
			>
				<view class="count">{{ formatCount(item.playCount) }}</view>
				<view class="duration pr-2">{{
					formatDuration(item.mv.videos[0].duration)
				}}</view>
			</view>
		</view>
		<view
			class="content text-gray-500 text-28 mt-1 line-height-4.5 van-multi-ellipsis--l2"
		>
...
import { IData } from '../../pages/home-video/types'
// interface IData {
// 	id: number
// 	cover: string
// 	name: string
